...
> ```
>
> The config workflow ([.github/workflows/config.yml](.github/workflows/config.yml)) skips the template push by default, gated on an `SMTP_CONFIGURED` repo variable (Settings → Secrets and variables → Actions → Variables tab) that's unset out of the box — migrations (a separate workflow, [migrate.yml](.github/workflows/migrate.yml)) deploy independently either way, so this never blocks shipping. It does mean **forgot/reset password stays broken in production until you turn it on**: upgrade to Supabase Pro, or configure a custom SMTP provider (Resend's free tier — 3,000 emails/month — works fine and unblocks the push). The SMTP route also means owning a domain — providers only deliver to arbitrary recipients from a domain you've verified via DNS records. Set up SMTP under **Authentication → Emails → SMTP Settings**, then add `SMTP_CONFIGURED` = `true` as a repo variable and re-run that workflow — see [Configure a real SMTP provider before you launch](#deploy-to-vercel). It's a repo variable rather than something committed in the file so a fork of this template never inherits whatever value your own deployment happens to be running with.
...
**The config workflow only actually pushes once you've set up custom SMTP.** It's gated behind an `SMTP_CONFIGURED` repo variable (Settings → Secrets and variables → Actions → **Variables** tab) that's unset by default — free-tier Supabase rejects `config push`'s email template changes outright (`Email template modification is not available for free tier projects using the default email provider`), so rather than let that fail the job on every push, the push step is cleanly *skipped* (a warning, not a failure) until the variable is set to `true`. It's a repo variable rather than a value committed in `config.yml` specifically so a fresh fork of this template never inherits whatever your own deployment happens to be running with. The migrations workflow is entirely separate and always runs regardless — it isn't gated by this at all. See the [Prerequisites](#0-prerequisites) caveat and [What happens on sign-up](#what-happens-on-sign-up) above for the full SMTP setup (Resend, port gotchas, `site_url`, testing). Once SMTP is configured and the variable is set, trigger it via the repo's Actions tab → "Deploy Supabase Config" → **Run workflow** (a manual `workflow_dispatch` trigger, no new commit needed).
...
└── config.yml                  # Pushes supabase/config.toml + templates/** to production (gated by the SMTP_CONFIGURED repo variable)
```
